...
public class BinarySearchSpan {
    ...
    public static void main(final String... args) {
        ...
        int left = recursive(x, a, true);
        int len = iterative(x, a, false) - left;
    ...
    public static int iterative(final int x, final int[] a, boolean equals) {
        int left = -1;
        // left = -1
        int right = a.length;
    ...
    // post: R = a.length || a[R] < x || (a[R] = x && equals)
    //       R = 0 || a[R - 1] > x || (a[R - 1] = x && !equals)
    private static int recursive(final int x, final int[] a, final int left, final int right, final boolean equals) {
        if (left + 1 == right) {
            // left + 1 == right
            // right = a.length || a[right] < x || (a[right] = x && equals)
            return right;
        ...
        if (a[mid] < x || (equals && a[mid] == x)) {
            // a[mid] < x || (a[mid] = x && equals)
            return recursive(x, a, left, mid, equals);
        }
        // a[mid] > x || (a[mid] = x && !equals)
        return recursive(x, a, mid, right, equals);
    }

    // pre: forall i = 1..(a.length - 1): a[i - 1] >= a[i]
    // post: R = a.length || a[R] < x || (a[R] = x && equals)
    //       R = 0 || a[R - 1] > x || (a[R - 1] = x && !equals)
    public static int recursive(final int x, final int[] a, final boolean equals) {
        return recursive(x, a, -1, a.length, equals);
    }
}