...
public class ExceptionsTest extends ParserTest {
    ...
    public static final Variable<Integer> VX = new Variable<>("x");
    public static final Variable<Integer> VY = new Variable<>("y");
    ...
    private void testParsingErrors() {
        for (final Op<String> op : parsingTest) {
            try {
                new ExpressionParser<Integer>(new IntegerOperation()).parse(op.f);
    ...
    protected static void testOverflow() {
        ...
        testOverflow((a, b) -> a + b, "+", new Add<Integer>(VX, VY, new IntegerOperation()));
        testOverflow((a, b) -> a - b, "-", new Subtract<Integer>(VX, VY, new IntegerOperation()));
        testOverflow((a, b) -> a * b, "*", new Multiply<Integer>(VX, VY, new IntegerOperation()));
        testOverflow((a, b) -> b == 0 ? Long.MAX_VALUE : a / b, "/", new Divide<Integer>(VX, VY, new IntegerOperation()));
        testOverflow((a, b) -> -b, "<- ignore first argument, unary -", new Negate<Integer>(VY, new IntegerOperation()));
    }

    protected static void testOverflow(final LongBinaryOperator f, final String op, final TripleExpression<Integer> expression) {
    ...
    protected TripleExpression<Integer> parse(final String expression, final boolean reparse) {
        ...
        counter.nextTest();
        try {
            final TripleExpression<Integer> parse = parser.parse(expression);
            counter.passed();
            return parse;
        } catch (final Exception e) {
