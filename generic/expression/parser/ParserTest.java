...
public class ParserTest extends BaseTest {
    ...
    protected TripleExpression<Integer> parse(final String expression, final boolean reparse) {
        try {
            final ExpressionParser<Integer> parser = new ExpressionParser<>(new IntegerOperation());
            if (reparse) {
                counter.nextTest();
            ...
            counter.nextTest();
            final CommonExpression<Integer> result = parser.parse(expression);
            counter.passed();
            return result;
